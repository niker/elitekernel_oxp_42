/*
 * arch/arm/mach-tegra/board-enrc2u.c
 *
...
#ifdef CONFIG_BT
static struct platform_device enrc2u_rfkill = {
	.name = "enrc2u_rfkill",
	.id = -1,
};
#endif

static __initdata struct tegra_clk_init_table enrc2u_clk_init_table[] = {
...
static struct tegra_i2c_platform_data enrc2u_i2c1_platform_data = {
	.adapter_nr	= 0,
	.bus_count	= 1,
...
static struct tegra_i2c_platform_data enrc2u_i2c2_platform_data = {
	.adapter_nr	= 1,
	.bus_count	= 1,
	.bus_clk_rate	= { 384000, 0 },
...
static struct tegra_i2c_platform_data enrc2u_i2c3_platform_data = {
	.adapter_nr	= 2,
	.bus_count	= 1,
	.bus_clk_rate	= { 384000, 0 },
...
static struct tegra_i2c_platform_data enrc2u_i2c4_platform_data = {
	.adapter_nr	= 3,
	.bus_count	= 1,
...
static struct tegra_i2c_platform_data enrc2u_i2c5_platform_data = {
	.adapter_nr	= 4,
	.bus_count	= 1,
	.bus_clk_rate	= { 100000, 0 },
...
static struct htc_asoc_platform_data enrc2u_audio_pdata;

static struct platform_device enrc2u_audio_device = {
	.name	= "tegra-snd-aic3008",
	.id	= 0,
	.dev	= {
		.platform_data  = &enrc2u_audio_pdata,
	},
};

#if (defined(CONFIG_SND_AMP_TFA9887))
static struct tfa9887_platform_data enrc2u_speaker_amp_data = {
...
static struct i2c_board_info i2c_speaker_amp_devices[] = {
	{
		...
		.platform_data = &enrc2u_speaker_amp_data,
		.irq = -1,
	},
};
#endif
...
static struct spi_board_info enrc2u_spi_board_info_rawchip[] __initdata = {
       {
...
EXPORT_SYMBOL_GPL(enrc2u_spi_board_info_rawchip);
...
static void enrc2u_i2c_init(void)
{
	tegra_i2c_device1.dev.platform_data = &enrc2u_i2c1_platform_data;
	tegra_i2c_device2.dev.platform_data = &enrc2u_i2c2_platform_data;
	tegra_i2c_device3.dev.platform_data = &enrc2u_i2c3_platform_data;
	tegra_i2c_device4.dev.platform_data = &enrc2u_i2c4_platform_data;
	tegra_i2c_device5.dev.platform_data = &enrc2u_i2c5_platform_data;
...
static struct platform_device *enrc2u_uart_devices[] __initdata = {
	&tegra_uarta_device,
	&tegra_uartb_device,
	&tegra_uartc_device,
	&tegra_uartd_device,
...
static struct tegra_uart_platform_data enrc2u_uart_pdata;
static struct tegra_uart_platform_data enrc2u_loopback_uart_pdata;

#ifdef CONFIG_SERIAL_TEGRA_BRCM
static struct tegra_uart_platform_data enrc2u_brcm_uart_pdata;
#endif
...
static void __init uart_debug_init(void)
{
	...
	enrc2u_uart_devices[0] = &debug_uarta_device;
...
static void __init enrc2u_uart_init(void)
{
	int i;
	struct clk *c;
	...
	enrc2u_uart_pdata.parent_clk_list = uart_parent_clk;
	enrc2u_uart_pdata.parent_clk_count = ARRAY_SIZE(uart_parent_clk);

/* GPS use UARTE
	enrc2u_loopback_uart_pdata.parent_clk_list = uart_parent_clk;
	enrc2u_loopback_uart_pdata.parent_clk_count =
						ARRAY_SIZE(uart_parent_clk);
	enrc2u_loopback_uart_pdata.is_loopback = true;
*/

	tegra_uarta_device.dev.platform_data = &enrc2u_uart_pdata;
	tegra_uartb_device.dev.platform_data = &enrc2u_uart_pdata;
	tegra_uartc_device.dev.platform_data = &enrc2u_uart_pdata;
	tegra_uartd_device.dev.platform_data = &enrc2u_uart_pdata;
	tegra_uarte_device.dev.platform_data = &enrc2u_uart_pdata; // Used for BRCM GPS.

#ifdef CONFIG_SERIAL_TEGRA_BRCM
	enrc2u_brcm_uart_pdata = enrc2u_uart_pdata;
	enrc2u_brcm_uart_pdata.bt_wakeup_pin_supported = 1;
	enrc2u_brcm_uart_pdata.bt_wakeup_pin = ENRC2U_GPIO_BT_WAKE;
	enrc2u_brcm_uart_pdata.host_wakeup_pin = ENRC2U_GPIO_BT_HOST_WAKE;
	tegra_uartc_device.dev.platform_data = &enrc2u_brcm_uart_pdata;
	tegra_uartc_device.name = "tegra_uart_brcm"; /* for brcm */
#endif

/* GPS uses UARTE
	// UARTE is used for loopback test purpose
	tegra_uarte_device.dev.platform_data = &enrc2u_loopback_uart_pdata;
*/
	...
	platform_add_devices(enrc2u_uart_devices,
				ARRAY_SIZE(enrc2u_uart_devices));
}
...
static struct tegra_spi_platform_data enrc2u_spi_pdata = {
	.is_dma_based		= true,
...
static void __init enrc2u_spi_init(void)
{
	int i;
	struct clk *c;
	...
	enrc2u_spi_pdata.parent_clk_list = spi_parent_clk;
	enrc2u_spi_pdata.parent_clk_count = ARRAY_SIZE(spi_parent_clk);

	spi_register_board_info(spi_board_info_audio, ARRAY_SIZE(spi_board_info_audio));
	spi_register_board_info(enrc2u_spi_board_info_rawchip, ARRAY_SIZE(enrc2u_spi_board_info_rawchip));
	platform_device_register(&tegra_spi_device2);
	tegra_spi_device4.dev.platform_data = &enrc2u_spi_pdata;
...
static struct platform_device *enrc2u_devices[] __initdata = {
	&tegra_pmu_device,
	&tegra_rtc_device,
	&tegra_udc_device,
...
static struct synaptics_i2c_rmi_platform_data edge_ts_3k_data_XB[] = {
	{
		...
		.config = {
			...
			0x0C,0x03,0x10,0x03,0xEC,0x4D,0x71,0x51,0xF8,0xA7,
			0xC8,0xAF,0x00,0x50,0x00,0x00,0x00,0x00,0x0A,0x04,
			...
			0x74,0x1E,0x05,0x20,0x02,0xFA,0x00,0x80,0x03,0x0E,
...
static int __init enrc2u_touch_init(void)
{
	int retval = 0;
	struct board_info BoardInfo;
	pr_info("[TP]Enter enrc2u_touch_init\n");
...
static int enrc2u_usb_hsic_postsupend(void)
{
	pr_debug("%s\n", __func__);
...
static int enrc2u_usb_hsic_preresume(void)
{
	pr_debug("%s\n", __func__);
#ifdef CONFIG_TEGRA_BB_XMM_POWER
...
static int enrc2u_usb_hsic_phy_ready(void)
{
	pr_debug("%s\n", __func__);
#ifdef CONFIG_TEGRA_BB_XMM_POWER
...
static int enrc2u_usb_hsic_phy_off(void)
{
	pr_debug("%s\n", __func__);
#ifdef CONFIG_TEGRA_BB_XMM_POWER
...
static struct tegra_usb_phy_platform_ops hsic_xmm_plat_ops = {
	.post_suspend = enrc2u_usb_hsic_postsupend,
	.pre_resume = enrc2u_usb_hsic_preresume,
	.port_power = enrc2u_usb_hsic_phy_ready,
	.post_phy_off = enrc2u_usb_hsic_phy_off,
};
...
static void enrc2u_usb_init(void)
{
...
static void config_tegra_usb_reset_wdt_gpios()
{
	...
	pr_info("[CABLE] %s:\n", __func__);
	/*
	** TEGRA_GPIO_PZ4:RESET_EN_CLR L:clear WDT (for XA)
	** TEGRA_GPIO_PP4:RESET_EN_CLR L:clear WDT (for XB)
	...
	*/
	...
	} else if (htc_get_pcbid_info() == PROJECT_PHASE_XA) {
		if (board_get_sku_tag() != 0x34600) {
			ret = gpio_request(TEGRA_GPIO_PZ4, "RESET_EN_CLR");
			if (ret < 0) {
				pr_err("%s: gpio_request failed %d\n", __func__, ret);
				return;
			}
			if (gpio_direction_input(TEGRA_GPIO_PZ4) < 0) {
				pr_info("[CABLE] %s: TEGRA_GPIO_PZ4(RESET_EN_CLR) dir NG\n", __func__);
				return;
			}
			...
			ret = gpio_direction_output(TEGRA_GPIO_PS6, 0);
			if (ret < 0) {
				pr_err("[CABLE] %s: gpio_direction_output failed %d\n", __func__, ret);
				gpio_free(TEGRA_GPIO_PS6);
				return;
			}
			tegra_gpio_enable(TEGRA_GPIO_PS6);
...
{
	if (board_get_sku_tag() == 0x34600) /* enrc2b */
		cable_detect_pdata.enable_reset_wdt = false;
	platform_device_register(&cable_detect_device);
}


static struct platform_device *enrc2u_audio_devices[] __initdata = {
	&tegra_ahub_device,
	&tegra_dam_device0,
	&tegra_dam_device1,
	&tegra_dam_device2,
...
static void enrc2u_audio_init(void)
{
	enrc2u_audio_codec_init(&enrc2u_audio_pdata);

	platform_add_devices(enrc2u_audio_devices,
			ARRAY_SIZE(enrc2u_audio_devices));
}

static void enrc2u_gps_init(void)
{
	int rc;
...
static void enrc2u_modem_init(void)
{
...
static __initdata struct tegra_pingroup_config enrc2u_pinmux_modem[] = {
...
#if 0
static __initdata struct tegra_pingroup_config enrc2u_pinmux_modem_XB[] = {
    DEFAULT_PINMUX(KB_ROW13,        RSVD,            PULL_DOWN, NORMAL,     INPUT),     //MDM_BB2AP_RST2
};
#endif

static void enrc2u_baseband_init(void)
{
#if 0
	...
	tegra_pinmux_config_table(enrc2u_pinmux_modem, ARRAY_SIZE(enrc2u_pinmux_modem));
	...
	{
		tegra_pinmux_config_table(enrc2u_pinmux_modem_XB, ARRAY_SIZE(enrc2u_pinmux_modem_XB));
	}
#endif
...
bool enr_swResetCheck()
{
	...
	if ( machine_is_enrc2u() && ((pcbid < PROJECT_PHASE_A ) || ( (pcbid == PROJECT_PHASE_A) && (engineer_id == 0x01 ))))
		return true;
	else
		return false;
}
...
static void __init enrc2u_init(void)
{
	...
	tegra_clk_init_from_table(enrc2u_clk_init_table);
	enrc2u_pinmux_init();
	enrc2u_i2c_init();
	enrc2u_spi_init();
	enrc2u_uart_init();
	...
	platform_add_devices(enrc2u_devices, ARRAY_SIZE(enrc2u_devices));
	if (!is_tegra_debug_uartport_hs()) {
	...
	enrc2u_audio_init();
	enrc2u_gps_init();
	enrc2u_baseband_init();
	enrc2u_panel_init();
	enrc2u_emc_init();
	enrc2u_sensors_init();
	enrc2u_cam_init();
	enrc2u_suspend_init();
	enrc2u_bpc_mgmt_init();
...
static void __init enrc2u_reserve(void)
{
...
MACHINE_START(ENRC2U, "enrc2u")
	.boot_params    = 0x80000100,
	.map_io         = tegra_map_common_io,
	.reserve        = enrc2u_reserve,
	.init_early	= tegra_init_early,
	.init_irq       = tegra_init_irq,
	.timer          = &tegra_timer,
	.init_machine   = enrc2u_init,
MACHINE_END
