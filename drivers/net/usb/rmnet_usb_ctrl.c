...
//--------------------------------------------------------
#ifdef HTC_DEBUG_QMI_TX_STUCK
static struct workqueue_struct *rmnet_usb_ctrl_read_kill_urb_queue = NULL;
static struct urb	*rcvurb_expired = NULL;
static void rmnet_usb_ctrl_read_kill_urb_thread(struct work_struct *work)
{
	if (rcvurb_expired) {
		pr_info("[%s] +kill urb %p\n", __func__, rcvurb_expired);
		usb_kill_urb(rcvurb_expired);
		pr_info("[%s] -kill urb %p\n", __func__, rcvurb_expired);
		rcvurb_expired = NULL;
	}
}
static DECLARE_WORK(rmnet_usb_ctrl_read_kill_urb_work, rmnet_usb_ctrl_read_kill_urb_thread);

static void rmnet_usb_ctrl_read_fatal_timer_expire(unsigned long unused)
{
	trace_printk("[%s]\n", __func__);
	tracing_off();
...
static DEFINE_TIMER(rmnet_usb_ctrl_read_fatal_timer, rmnet_usb_ctrl_read_fatal_timer_expire, 0, 0);
...
static void rmnet_usb_ctrl_read_timer_expire(unsigned long data)
{
	...
	dev->is_rcv_timeout = true;

	rcvurb_expired = dev->rcvurb;
	queue_work(rmnet_usb_ctrl_read_kill_urb_queue, &rmnet_usb_ctrl_read_kill_urb_work);

	//Kris_test: trigger fatal after 5 seconds
...
static void notification_available_cb(struct urb *urb)
{
	...
	if (get_radio_flag() & 0x0001)//SSD_RIL:For TX data still debug
		pr_info("[RMNET] ncb\n");
...
//--------------------------------------------------------
#ifdef HTC_DEBUG_QMI_TX_STUCK
		mod_timer(&dev->rcv_timer, jiffies + msecs_to_jiffies(3000));
#endif	//HTC_DEBUG_QMI_TX_STUCK
//--------------------------------------------------------

		usb_mark_last_busy(udev); //+Sophia:0703:reset auto-suspend timer in rx callback function
		if (status) {
			pr_info("[RMNET] %s: Error submitting Read URB %d\n", __func__, status);
			dev_err(dev->devicep,
...
//--------------------------------------------------------
		...
		if (!dev->resp_available) {
			pr_info("%s wake_up open_wait_queue\n", __func__);
...
//--------------------------------------------------------
	...
	if (status) {
		pr_info("[RMNET]%s: Error re-submitting Int URB %d\n", __func__, status);
...
//--------------------------------------------------------
	...
	if (dev->is_rcv_timeout) {
		dev->is_rcv_timeout = false;
		dev_err(dev->devicep, "[RMNET] %s(%d) urb->status %d !!!\n", __func__, __LINE__, urb->status);
...
//--------------------------------------------------------
	...
	if (!cpkt_size) {
		dev->zlp_cnt++;
		pr_info("[RMNET] %s: zero length pkt received\n", __func__);
...
//--------------------------------------------------------
	...
	context->timer.data = sndurb;
	context->start_jiffies = jiffies;
	mod_timer(&context->timer, jiffies + msecs_to_jiffies(5000));
#endif	//HTC_DEBUG_QMI_TX_STUCK
//--------------------------------------------------------
...
//--------------------------------------------------------
#ifdef HTC_DEBUG_QMI_TX_STUCK
	rmnet_usb_ctrl_read_kill_urb_queue = create_singlethread_workqueue("rmnet_usb_ctrl_read_kill_urb_queue");
	if (!rmnet_usb_ctrl_read_kill_urb_queue) {
		pr_err("%s: could not create workqueue rmnet_usb_ctrl_read_kill_urb_queue\n", __func__);
	}
#endif	//HTC_DEBUG_QMI_TX_STUCK
//--------------------------------------------------------
