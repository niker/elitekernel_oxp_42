/*
 * drivers/tty/serial/tegra_hsuart_brcm.c
 *
...
#define UARTDM_TO_TEGRA(uart_port) \
        container_of((uart_port), struct tegra_uart_port, uport)
...
#ifdef USE_BCM_BT_CHIP /* USE_BCM_BT_CHIP */
	int wakeup_irq;

	enum broadcom_bt_state bt_state;
...
static struct attribute_group a2dp_serial_hs_attribute_group = {
	.attrs = a2dp_serial_hs_attributes,
};

static ssize_t show_a2dp_tuning(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static struct attribute *opp_serial_hs_attributes[] = {
	&dev_attr_opp_tuning.attr,
	NULL,
};

static struct attribute_group opp_serial_hs_attribute_group = {
	.attrs = opp_serial_hs_attributes,
};

static ssize_t show_opp_tuning(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", opp_tuning_state);
}

static ssize_t store_opp_tuning(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	char in_char[] = "0";
	...
	if (0 == opp_tuning_state) {
		pm_qos_update_request(&opp_cpu_minfreq_req, (s32)PM_QOS_CPU_FREQ_MIN_DEFAULT_VALUE);
		pr_info("pm_qos_update_request - PM_QOS_CPU_FREQ_MIN_DEFAULT_VALUE");
	}
	else if (1 == opp_tuning_state) {
		pm_qos_update_request(&opp_cpu_minfreq_req, (s32)OPP_CPU_FREQ_MIN);
		pr_info("pm_qos_update_request - OPP_CPU_FREQ_MIN");
	}
...
static irqreturn_t bluesleep_hostwake_isr(int irq, void *dev)
{
...
#ifdef BCM_BT_DEBUG
	dev_info(t->uport.dev, "[SER_BRCM]bluesleep_hostwake_isr\n");
#endif
	...
	if (t->host_wakeup_level == 0) { /* host_wake is high */
		...
		if (t->is_brcm_rx_wake_locked == 0) {
			t->is_brcm_rx_wake_locked = 1;
			wake_lock(&t->brcm_rx_wake_lock);
		}

		t->host_wakeup_level = 0;
		irq_set_irq_type(t->wakeup_irq, IRQF_TRIGGER_HIGH);
	}
...
static int tegra_uart_init_rx_dma_buffer(struct tegra_uart_port *t)
{
	...
	t->rx_dma_req.req_sel = dma_req_sel_brcm[t->uport.line];
...
static int tegra_startup(struct uart_port *u)
{
	...
	if (t->use_tx_dma) {
		...
		t->tx_dma_req.req_sel = dma_req_sel_brcm[t->uport.line];
		t->tx_dma_req.dev = t;
		t->tx_dma_req.size = 0;
	...
	if (t->bt_state == BT_ON_SYS_IDLE) {
#ifdef BCM_BT_DEBUG
		dev_info(t->uport.dev, "[SER_BRCM] BT_ON_SYS_IDLE\n");
#endif //BCM_BT_DEBUG
		/* disable irq wakeup when system is up */
		disable_irq_wake(t->wakeup_irq);
		//free_irq(t->wakeup_irq, t);

	} else { /* BT_OFF_SYS_IDLE */
#ifdef BCM_BT_DEBUG
		dev_info(t->uport.dev, "[SER_BRCM] BT_OFF_SYS_IDLE -> BT_ON_SYS_IDLE\n");
#endif //BCM_BT_DEBUG
		t->host_wakeup_level = 1;
		t->is_brcm_rx_wake_locked = 0;
	...
	t->bt_state = BT_ON_SYS_IDLE;

#endif //USE_BCM_BT_CHIP

#ifdef BCM_BT_DEBUG
	dev_info(t->uport.dev, "[SER_BRCM] -tegra_startup\n");
#endif //BCM_BT_DEBUG
...
static void tegra_shutdown(struct uart_port *u)
{
...
#ifdef BCM_BT_DEBUG
		dev_info(t->uport.dev, "[SER_BRCM] BT_ON_SYS_SUSPEND\n");
#endif //BCM_BT_DEBUG
		ret = enable_irq_wake(t->wakeup_irq);
		if (ret < 0) {
			dev_err(u->dev, "[SER_BRCM]Couldn't enable BT_HOST_WAKE as wakeup interrupt, (errno = %d)", ret);
			free_irq(t->wakeup_irq, NULL);
		}

		t->bt_state = BT_ON_SYS_SUSPEND;//suspend BT
	}
	...
	wake_unlock(&t->brcm_tx_wake_lock);
	wake_unlock(&t->brcm_rx_wake_lock);
...
#ifdef BCM_BT_DEBUG
	dev_info(u->dev, "[SER_BRCM]tegra_ioctl cmd : %x\n", cmd);
#endif //BCM_BT_DEBUG
...
static int __init tegra_uart_probe(struct platform_device *pdev)
{
	...
	u->fifosize = 32;
		...
		&a2dp_serial_hs_attribute_group);
		if (ret)
...
#ifdef OPP_TUNING_SUPPORTED
		ret = sysfs_create_group(&pdev->dev.kobj,
		&opp_serial_hs_attribute_group);
		if (ret)
			printk(KERN_ERR "%s reg attr fail!!", __func__);

		SERIAL_HS_SET_DEVICE_ATTR(opp_tuning, 0644, show_opp_tuning, store_opp_tuning);
#endif
...
#ifdef USE_BCM_BT_CHIP /* BRCM_BT */

	wake_lock_init(&t->brcm_tx_wake_lock, WAKE_LOCK_SUSPEND,
			"tegra_serial_hs_brcm_tx");

	wake_lock_init(&t->brcm_rx_wake_lock, WAKE_LOCK_SUSPEND,
			"tegra_serial_hs_brcm_rx");

	if (pdata == NULL) {
		pr_info("[SER_BRCM] %s: could not find pdev\n", __func__);
		t->wakeup_irq = -1;
	} else {
		t->wakeup_irq = gpio_to_irq(pdata->host_wakeup_pin);

		if (unlikely(t->wakeup_irq < 0)) {
			pr_err("[SER_BRCM] %s: Failed to add wakeup irq\n", __func__);
			return -ENXIO;
		}
	}

	if (pdata->bt_wakeup_pin_supported) {
		t->bt_wakeup_pin_supported = pdata->bt_wakeup_pin_supported;
		t->bt_wakeup_pin = pdata->bt_wakeup_pin;
		ret = gpio_request(t->bt_wakeup_pin, "brcm_bt_ext_wake");
		tegra_gpio_enable(t->bt_wakeup_pin);
		/* configure ext_wake as output mode*/
		ret = gpio_direction_output(t->bt_wakeup_pin, T_LOW);
		if (ret < 0) {
			pr_err("gpio-keys: failed to configure output"
			" direction for GPIO %d, error %d\n",
			t->bt_wakeup_pin, ret);
			gpio_free(t->bt_wakeup_pin);
			goto fail;
		}

		t->bt_wakeup_level = 1;
		t->bt_wakeup_assert_inadvance = 0;
		t->host_wakeup_pin = pdata->host_wakeup_pin;
		ret = gpio_request(t->host_wakeup_pin, "brcm_bt_host_wake");
		if (ret)
			goto fail;

		/* configure host_wake as input */
		tegra_gpio_enable(t->host_wakeup_pin);
		ret = gpio_direction_input(t->host_wakeup_pin);
		if (ret < 0) {
			pr_err("gpio-keys: failed to configure input"
			" direction for GPIO %d, error %d\n",
			t->host_wakeup_pin, ret);
			gpio_free(t->host_wakeup_pin);
			goto fail;
		}

		t->host_want_sleep = 1;
		t->host_wakeup_level = 1;
		t->bt_state = BT_OFF_SYS_IDLE;
	}
...
static int __devexit tegra_uart_remove(struct platform_device *pdev)
{
	...
	/* destroy tx wakelock */
	wake_lock_destroy(&t->brcm_tx_wake_lock);
	/* destroy rx wakelock */
	wake_lock_destroy(&t->brcm_rx_wake_lock);

	pr_info("[SER_BRCM] Unregistered UART port %s%d\n",
			tegra_uart_driver.dev_name, u->line);
...
static int tegra_uart_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (t->bt_state == BT_ON_SYS_IDLE) {
		...
		dev_info(t->uport.dev, "[SER_BRCM] BT_ON_SYS_IDLE -> BT_ON_SYS_SUSPEND\n");
#endif //BCM_BT_DEBUG
		t->bt_state = BT_ON_SYS_SUSPEND;
	} else { /* BT_OFF_SYS_IDLE */
#ifdef BCM_BT_DEBUG	/* bt for brcm */
		dev_info(t->uport.dev, "[SER_BRCM] BT_OFF_SYS_IDLE -> BT_OFF_SYS_SUSPEND\n");
#endif //BCM_BT_DEBUG
		t->bt_state = BT_OFF_SYS_SUSPEND;
	}
...
static int tegra_uart_resume(struct platform_device *pdev)
{
	...
	if (t->bt_state == BT_OFF_SYS_SUSPEND) {
#ifdef BCM_BT_DEBUG	/* bt for brcm */
		dev_info(t->uport.dev, "[SER_BRCM] BT_OFF_SYS_SUSPEND -> BT_OFF_SYS_IDLE\n");
#endif //BCM_BT_DEBUG
		t->bt_state = BT_OFF_SYS_IDLE;
	} else { /* BT_ON_SYS_SUSPEND */
#ifdef BCM_BT_DEBUG	/* bt for brcm */
		dev_info(t->uport.dev, "[SER_BRCM] BT_ON_SYS_SUSPEND -> BT_ON_SYS_IDLE\n");
#endif //BCM_BT_DEBUG
		t->bt_state = BT_ON_SYS_IDLE;
	}
#endif
	...
	dev_info(t->uport.dev, "[SER_BRCM] -tegra_uart_resume end\n");
	return 0;
}
...
void tegra_brcm_uart_set_mctrl(struct uart_port *uport, unsigned int mctrl)
{
	unsigned long flags;
	struct tegra_uart_port *t;
...
static struct platform_driver tegra_uart_platform_driver __refdata = {
	...
	.driver		= {
		.name	= "tegra_uart_brcm"
	}
};

static int __init tegra_uart_brcm_init(void)
{
	int ret;
...
#ifdef OPP_TUNING_SUPPORTED
	pm_qos_add_request(&opp_cpu_minfreq_req, PM_QOS_CPU_FREQ_MIN, (s32)PM_QOS_CPU_FREQ_MIN_DEFAULT_VALUE);
#endif
...
static void __exit tegra_uart_brcm_exit(void)
{
...
module_init(tegra_uart_brcm_init);
module_exit(tegra_uart_brcm_exit);
